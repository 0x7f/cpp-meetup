...
void body_handler(OpenRtbRequest* req,
    ...
    if (ec == boost::asio::error::eof || bufLen == 0) {
        ...
        int rc = bidrequest_parser_on_chunk(req->state, const_cast<char*>(buf), bufLen);
        if (rc != 0) {
...
void connection_handler(HTTPP::HTTP::Connection* connection) {
    ...
    const auto it = std::find_if(headers.begin(), headers.end(), [](const HTTPP::HTTP::HeaderRef& s) {
